...
{
public:
    rotate_z(shared_ptr<hittable> p, double angle) : object(p), theta(angle)
    {
        auto radians = degrees_to_radians(angle);
        sin_theta = std::sin(radians);
        cos_theta = std::cos(radians);
    ...
    bool hit(const ray& r, interval ray_t, hit_record& rec) const override
    {
         ...
         * cos(theta) -sin(theta)
         * sin(theta) cos(theta)
         */ //to be multiplied with vector (x, y)
        /*
         * new x: cos(theta)x - sin(theta)y
         * new y: sin(theta)x + cos(theta)y
         */
        //but here we want to rotate by -theta, cos(-theta) = cos(theta) and sin(-theta) = -sin(theta)
        auto origin = point3(
    ...
    std::string to_string() const override
    {
        return "Base: " + object->to_string() + " | Rotated by " + std::to_string(theta) + " around z axis";
    }
private:
    shared_ptr<hittable> object;
    double theta;
